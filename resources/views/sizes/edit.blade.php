...
<div class="space-y-6">
    <div class="mb-6 flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold">Chỉnh Sửa Kích Thước</h1>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                ...
                placeholder="Mô tả chi tiết về kích thước...">{{ old('description', $size->description) }}</textarea>
            @error('description')
                <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
            @else
                <p class="text-xs text-gray-400 mt-1">{{ strlen(old('description', $size->description)) }}/500 ký tự</p>
            @enderror
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 space-y-4">
        <div class="flex items-center justify-between">
            ...
            <input type="checkbox" name="status" id="status" value="active"
                ...
                {{ old('status', $size->status) == 'active' ? 'checked' : '' }}>
        </div>
        <p class="text-sm text-gray-400">
            {{ old('status', $size->status) == 'active'
