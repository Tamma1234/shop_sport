<!-- Blog -->
<section class="themesFlat">
    <div class="container">
        <div class="sect-title text-center wow fadeInUp">
            <h1 class="s-title mb-8">{{ $blogSection['title'] ?? 'Our Blog' }}</h1>
            <p class="s-subtitle h6">{{ $blogSection['subtitle'] ?? 'Up to 50% off Lorem ipsum dolor sit amet, consectetur adipiscing elit' }}</p>
        </div>
        
        @if(isset($blogPosts) && count($blogPosts) > 0)
        <div dir="ltr" class="swiper tf-swiper" data-preview="3" data-tablet="3" data-mobile-sm="2" data-mobile="1" data-space-lg="48"
            data-space-md="32" data-space="12" data-pagination="1" data-pagination-sm="2" data-pagination-md="3" data-pagination-lg="3">
            <div class="swiper-wrapper">
                @foreach($blogPosts as $index => $post)
                <!-- item {{ $index + 1 }} -->
                <div class="swiper-slide">
                    <div class="article-blog type-space-2 hover-img4 wow fadeInLeft" @if($index > 0) data-wow-delay="{{ $index * 0.1 }}s" @endif>
                        <a href="{{ $post['url'] ?? route('client.blog.detail', $post['slug'] ?? 'sample-post') }}" class="entry_image img-style4">
                            <img src="{{ $post['image'] ?? asset('images/blog/blog-' . (5 + $index) . '.jpg') }}" 
                                 data-src="{{ $post['image'] ?? asset('images/blog/blog-' . (5 + $index) . '.jpg') }}" 
                                 alt="{{ $post['title'] ?? 'Blog' }}" 
                                 class="lazyload aspect-ratio-0">
                        </a>
                        <div class="entry_tag">
                            <a href="{{ route('client.blog.grid') }}" class="name-tag h6 link">
                                {{ $post['date'] ?? 'March ' . ($index + 2) . ', 2025' }}
                            </a>
                        </div>
                        <div class="blog-content">
                            <a href="{{ $post['url'] ?? route('client.blog.detail', $post['slug'] ?? 'sample-post') }}" class="entry_name link h4">
                                {{ $post['title'] ?? 'Sample Blog Post ' . ($index + 1) }}
                            </a>
                            <p class="text h6">
                                {{ $post['excerpt'] ?? 'Lorem ipsum dolor sit amet, consectetur adipiscing elit in malesuada magna faucibus. Pellentesque eget finibus nunc.' }}
                            </p>
                            <a href="{{ $post['url'] ?? route('client.blog.detail', $post['slug'] ?? 'sample-post') }}" class="tf-btn-line">
                                {{ $post['read_more_text'] ?? 'Read more' }}
                            </a>
                        </div>
                    </div>
                </div>
                @endforeach
            </div>
            <div class="sw-dot-default tf-sw-pagination"></div>
        </div>
        @else
        <!-- Fallback content when no blog posts are provided -->
        <div dir="ltr" class="swiper tf-swiper" data-preview="3" data-tablet="3" data-mobile-sm="2" data-mobile="1" data-space-lg="48"
            data-space-md="32" data-space="12" data-pagination="1" data-pagination-sm="2" data-pagination-md="3" data-pagination-lg="3">
            <div class="swiper-wrapper">
                <!-- item 1 -->
                <div class="swiper-slide">
                    <div class="article-blog type-space-2 hover-img4 wow fadeInLeft">
                        <a href="{{ route('client.blog.detail', 'sample-post-1') }}" class="entry_image img-style4">
                            <img src="{{ asset('images/blog/blog-5.jpg') }}" data-src="{{ asset('images/blog/blog-5.jpg') }}" alt="Blog" class="lazyload aspect-ratio-0">
                        </a>
                        <div class="entry_tag">
                            <a href="{{ route('client.blog.grid') }}" class="name-tag h6 link">March 2, 2025</a>
                        </div>
                        <div class="blog-content">
                            <a href="{{ route('client.blog.detail', 'sample-post-1') }}" class="entry_name link h4">
                                Skin Whitening With New Technology
                            </a>
                            <p class="text h6">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit in malesuada magna faucibus. Pellentesque eget finibus
                                nunc.
                            </p>
                            <a href="{{ route('client.blog.detail', 'sample-post-1') }}" class="tf-btn-line">
                                Read more
                            </a>
                        </div>
                    </div>
                </div>
                <!-- item 2 -->
                <div class="swiper-slide">
                    <div class="article-blog type-space-2 hover-img4 wow fadeInLeft" data-wow-delay="0.1s">
                        <a href="{{ route('client.blog.detail', 'sample-post-2') }}" class="entry_image img-style4">
                            <img src="{{ asset('images/blog/blog-6.jpg') }}" data-src="{{ asset('images/blog/blog-6.jpg') }}" alt="Blog" class="lazyload aspect-ratio-0">
                        </a>
                        <div class="entry_tag">
                            <a href="{{ route('client.blog.grid') }}" class="name-tag h6 link">March 2, 2025</a>
                        </div>
                        <div class="blog-content">
                            <a href="{{ route('client.blog.detail', 'sample-post-2') }}" class="entry_name link h4">
                                Anti Aging With Vitamin C
                            </a>
                            <p class="text h6">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit in malesuada magna faucibus. Pellentesque eget finibus
                                nunc.
                            </p>
                            <a href="{{ route('client.blog.detail', 'sample-post-2') }}" class="tf-btn-line">
                                Read more
                            </a>
                        </div>
                    </div>
                </div>
                <!-- item 3 -->
                <div class="swiper-slide">
                    <div class="article-blog type-space-2 hover-img4 wow fadeInLeft" data-wow-delay="0.2s">
                        <a href="{{ route('client.blog.detail', 'sample-post-3') }}" class="entry_image img-style4">
                            <img src="{{ asset('images/blog/blog-7.jpg') }}" data-src="{{ asset('images/blog/blog-7.jpg') }}" alt="Blog" class="lazyload aspect-ratio-0">
                        </a>
                        <div class="entry_tag">
                            <a href="{{ route('client.blog.grid') }}" class="name-tag h6 link">March 2, 2025</a>
                        </div>
                        <div class="blog-content">
                            <a href="{{ route('client.blog.detail', 'sample-post-3') }}" class="entry_name link h4">
                                Miraculous Uses Of Aloe Vera
                            </a>
                            <p class="text h6">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit in malesuada magna faucibus. Pellentesque eget finibus
                                nunc.
                            </p>
                            <a href="{{ route('client.blog.detail', 'sample-post-3') }}" class="tf-btn-line">
                                Read more
                            </a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="sw-dot-default tf-sw-pagination"></div>
        </div>
        @endif
    </div>
</section>
<!-- /Blog --> 