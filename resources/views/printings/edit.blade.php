...
<div class="space-y-6">
    <div class="mb-6 flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold">Chỉnh Sửa In Ấn</h1>
            </div>
        </div>
    </div>

    <form action="{{ route('printings.update', $printing->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <div class="bg-white rounded-lg shadow p-6 space-y-6">
            <div>
                <h2 class="text-xl font-semibold">Thông Tin In Ấn <span class="text-sm text-gray-400">(Bắt buộc)</span></h2>
                <p class="text-gray-500 text-sm">Cập nhật thông tin cho loại in ấn</p>
            </div>
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Tên In Ấn *</label>
                <input type="text" name="name" id="name" value="{{ old('name', $printing->name) }}"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    ...
                    placeholder="Mô tả về loại in ấn...">{{ old('description', $printing->description) }}</textarea>
                @error('description')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
