...
<div class="space-y-6">
    <div class="mb-6 flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold">Sửa Quà Tặng</h1>
            </div>
        </div>
    </div>

    <form action="{{ route('gifts.update', $gift) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div>
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $gift->price) }}" min="0" step="1000"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    ...
                    placeholder="Mô tả chi tiết về quà tặng...">{{ old('description', $gift->description) }}</textarea>
                @error('description')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" id="status"
                    ...
                    <option value="active" @selected(old('status', $gift->status) == 'active')>Hoạt động</option>
                    <option value="inactive" @selected(old('status', $gift->status) == 'inactive')>Không hoạt động</option>
                </select>
                @error('status')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
