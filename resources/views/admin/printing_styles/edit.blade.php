...
<div class="space-y-6">
    <div class="mb-6 flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold">Chỉnh Sửa Kiểu In Ấn</h1>
            </div>
        </div>
    </div>
    <form action="{{ route('printing-styles.update', $printing_style->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        <div class="bg-white rounded-lg shadow p-6 space-y-6">
            <div>
                <h2 class="text-xl font-semibold">Thông Tin Kiểu In Ấn <span class="text-sm text-gray-400">(Bắt buộc)</span></h2>
                <p class="text-gray-500 text-sm">Cập nhật thông tin cho kiểu in ấn</p>
            </div>
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Tên Kiểu In *</label>
                <input type="text" name="name" id="name" value="{{ old('name', $printing_style->name) }}"
            ...
            <div>
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $printing_style->price) }}"
            ...
            <div>
                ...
                <select name="printing_id" id="printing_id" class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('printing_id') border-red-500 @enderror">
                    ...
                    @foreach($printings as $printing)
                        <option value="{{ $printing->id }}" {{ old('printing_id', $printing_style->printing_id) == $printing->id ? 'selected' : '' }}>{{ $printing->name }}</option>
                    @endforeach
                </select>
                @error('printing_id')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
